...
export default async (data: Handler) => {
  ...
  const mlmSettings = (await settings.has("mlmSettings"))
    ? JSON.parse((await settings.get("mlmSettings")) || "{}")
    : null;

  const mlmSystem = (await settings.has("mlmSystem"))
    ? (await settings.get("mlmSystem"))
    : null;
